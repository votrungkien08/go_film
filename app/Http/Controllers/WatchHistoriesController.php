...
class WatchHistoriesController extends Controller
{
    public function store(Request $request)
    {
        try {
            ...
            $request->validate([
                'episodes_id' => 'required|integer|exists:film_episodes,id', // Đã sửa từ episodes_id
                'progress_time' => 'required|integer|min:0',
            ]);

            $history = Watch_histories::updateOrCreate([
                'user_id' => $user->id,
                'episodes_id' => $request->episodes_id, // Đã sửa từ episodes_id
            ], [
                'watch_at' => now(),
            ...
            Log::info('Watch history stored successfully', [
                'user_id' => $user->id,
                'episodes_id' => $request->episodes_id, // Đã sửa từ episode_id
                'progress_time' => $request->progress_time,
            ]);
            return response()->json([
