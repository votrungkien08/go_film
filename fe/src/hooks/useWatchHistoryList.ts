...
export const useWatchHistoryList = () => {
    ...
    const fetchFilmEpisodes = async (episodeId: number) => {
        if (!episodeId) {
            console.warn(
                `Skipping fetchFilmEpisodes for invalid episodeId: ${episodeId}`
            );
            return null;
        }
        try {
            const response = await axios.get(
                `http://localhost:8000/api/episode/${episodeId}`,
                {
                    headers: {
        ...
        try {
            ...
            const enhancedHistories = await Promise.all(
                histories.map(async (history: WatchHistories) => {
                    const filmEpisodes = await fetchFilmEpisodes(
                        history.episodes_id
                    );
                    return {
                        ...history,
            ...
            const latestHistories = Object.values(
                enhancedHistories.reduce(
                    (acc: { [key: number]: EnhancedWatchHistory }, history) => {
                        if (
                            history.film &&
                            history.episode?.id &&
                            (!acc[history.film.id] ||
                                getEpisodeNumber(
                                    history.episode.episode_title
                                ) >
                                    getEpisodeNumber(
                                        acc[history.film.id].episode
                                            ?.episode_title || "FULL"
                                    ))
                        ) {
            ...
            const sortedHistories = latestHistories.sort(
                (a, b) =>
                    new Date(b.watch_at).getTime() -
                    new Date(a.watch_at).getTime()
            );
    ...
    // Function để xóa lịch sử khỏi state local
    const removeHistoryFromList = (historyId: number) => {
        setWatchHistory((prev) =>
            prev.filter((history) => history.id !== historyId)
        );
    };

    // Function để refresh danh sách lịch sử
